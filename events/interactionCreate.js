...
const { DEAL_CATEGORY_NAME, OWNER_IDS, COLORS, CRYPTOCURRENCIES } = require('../config');
      ...
      if (customId.startsWith('release_')) {
        ...
        await interaction.reply({
          ...
          embeds: [new EmbedBuilder()
            ...
            .setDescription(`<@${deal.buyer_id}> has released the funds!\n\n**Seller:** You may now withdraw your funds from the escrow wallet.\n**Deal ID:** ${dealId}`)
            .addFields(
              ...
              { name: '📬 Escrow Address', value: `\`${deal.wallet_address}\``, inline: false },
              { name: '⚠️ Withdrawal', value: 'Contact the **bot owner** to withdraw funds from the escrow wallet, or use `/send_funds` if you are the owner.' },
            )
            .setTimestamp()
          ],
        });
        ...
        if (wallet) {
          const walletMsg = await interaction.channel.send({
            embeds: [new EmbedBuilder()
              .setTitle('🔐 Escrow Wallet Info (Admin Only)')
              .setColor(COLORS.DARK)
              .setDescription('Funds are held at the address below. Owner use `/send_funds` to withdraw.')
              .addFields({ name: '📬 Address', value: `\`${wallet.address}\`` })
              .setTimestamp()
            ],
          });
          await walletMsg.pin().catch(() => {});
        }
